...
const BrandTrash = () => {
  ...
  const handleTrashClick = async (brandId) => {
    try {
      const productResponse = await axios.get(`https://localhost:44389/api/Brand/${brandId}`);
      ...
      const updateResponse = await axios.put(
        `https://localhost:44389/api/Brand/${brandId}`,brand,
        {
  ...
  const handleDeleteClick = async (brandId) => {
    try {
      const reponse = await axios.delete(`https://localhost:44389/api/Brand/${brandId}`,{
      ...
      if (error.response && error.response.status===404) {
        toast.error("Không thể xóa ! thương hiệu này hiện chứa nhiều sản phẩm hoặc Thương hiệu con.")
            ...
            <div className="col-sm-6">
              <ol className="breadcrumb float-sm-right">
                <li className="breadcrumb-item">
                  <Link to={`../admin/category`}>Thương hiệu</Link>
                </li>
              </ol>
            </div>
          </div>
